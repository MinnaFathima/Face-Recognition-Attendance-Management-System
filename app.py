...
def name():
    if request.method == "POST":
        ...
        while True:
            ...
            k = cv2.waitKey(1)
            if k % 256 == 27:
                # ESC pressed, close the window
                print("Escape hit, closing...")
                break
            elif k % 256 == 32:
                # SPACE pressed, capture the image and save the name
                ...
                # Saving the name to a CSV file when SPACE is pressed
                with open('attendance.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([name1, datetime.now().strftime('%H:%M:%S')])  # Save the name with timestamp
                    print(f"Name {name1} added to CSV.")
